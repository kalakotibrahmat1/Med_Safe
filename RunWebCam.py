...
main = tkinter.Tk()
main.title("QR Code with Block Chain Technology for Medical Device Ownership") #designing main screen
...
def validateDetails(data):
    ...
    for i in range(len(arr)-1):
        ...
        if array[0].strip() == data.strip():
            ...
            print(str(array[0])+" ****** "+data)   
            text.insert(END,'Equipment ID      : '+array[0]+"\n")
            text.insert(END,'Equipment Name    : '+array[1]+"\n")
            text.insert(END,'Equipment Price     : '+array[2]+"\n")
            text.insert(END,'Manufacturing Details      : '+array[3]+"\n")
            text.insert(END,'Company Details : '+array[4]+"\n")
    ...
    if flag == 0:
        text.delete('1.0', END)
        text.insert(END,"QR Verification Failed. QR Code does not exists")
        text.update_idletasks()
...
font = ('times', 13, 'bold')
title = Label(main, text='QR Code with Block Chain Technology for Medical Device Ownership')
